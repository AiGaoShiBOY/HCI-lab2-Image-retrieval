...
def tags_search(img_name):
    ...
    files = os.listdir('database/tags')
    flag = 0
    if os.path.exists(res_path):
        return
    else:
        for file in files:
            position = 'database/tags' + '/' + file
            fopen = open(position, 'r')
            ...
            for line in lines:
                ...
                if line == number:
                    flag = flag + 1
                    tokens = position.split("/")[-1]
                    types = tokens.split(".")[0]
                    f_res.write(types + '\n')
                    print(types)
                    f_res.close()
                    break
            if flag == 3: # prevent too many tags
                break
